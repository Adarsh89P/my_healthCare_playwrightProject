...
- **A hard reuse boundary.** `src/core/` contains zero application knowledge; `src/apps/<app>/` and
  `tests/<app>/` contain all of it. Testing a second product means adding those two directories -
  not editing the framework. `APP=<name> npm test` chooses which one runs.
...
├── tsconfig.json                     # strict + @core / @apps / @ai path aliases
...
│   │
│   └── apps/                         # ══ PROJECT-SPECIFIC — one folder per app ══
│       ├── _template/                #    copy this to onboard a new application
│       │   ├── config.ts             #    routes only; URLs come from .env
│       │   ├── fixtures/index.ts     #    extends the core base fixture
│       │   └── pages/ExamplePage.ts  #    starting point for a first page object
│       │
│       └── healthcare/               #    the CURA demo app, as a worked example
│           ├── config.ts             #    route constants
│           ├── fixtures/index.ts     #    page objects exposed as fixtures
│           ├── pages/                #    LoginPage, AppointmentPage
│           └── testdata/             #    typed facilities + message fixtures
│
└── tests/
    ├── unit/                         # framework-level: no browser, no network
    └── healthcare/                   # one folder per app, selected by APP=
        ├── setup/auth.setup.ts       #   signs in once, saves storageState
        ├── smoke/                    #   @smoke - the golden paths
        └── regression/               #   @regression - data-driven breadth
```
...
| A specific application      | `APP=healthcare npm test`           |
| Quality gate (what CI runs) | `npm run verify`                    |

### Pointing it at a different application

This is the part that makes the repo a framework rather than one project's tests.

```bash
cp -r src/apps/_template src/apps/myapp     # page objects + fixtures
mkdir -p tests/myapp/{smoke,regression}     # specs
echo "BASE_URL=https://myapp.example.com" >> .env.uat

APP=myapp npm test
```

`src/core` and `src/ai` are untouched by that. See
[`src/apps/_template/README.md`](src/apps/_template/README.md) for the full checklist.
...
Test data lives in `src/apps/<app>/testdata/*.ts`, not `*.json`. JSON is more familiar and tooling-agnostic;
